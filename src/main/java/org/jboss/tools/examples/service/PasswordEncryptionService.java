...
public class PasswordEncryptionService {
 ...
 public static boolean authenticate(String attemptedPassword, byte[] encSaltedPassword)
   throws NoSuchAlgorithmException, InvalidKeySpecException {
  // Encrypt the clear-text password using the same salt that was used to
  // encrypt the original password
 
  byte[] salt = new byte[8];
  System.arraycopy(encSaltedPassword, 20, salt, 0, 8);
  
  byte[] encryptedAttemptedPassword = getEncryptedPassword(attemptedPassword, salt);

  // Authentication succeeds if encrypted password that the user entered
  // is equal to the stored hash
  return Arrays.equals(encryptedAttemptedPassword, encSaltedPassword);
 }
 ...
 private static byte[] getEncryptedPassword(String password, byte[] salt)
  ...
  byte[] encSaltedPwd = concatenatePasswordAndSalt(f.generateSecret(spec).getEncoded(), salt);
